...
ZIP_SHP   = WORKDIR / "seccionado_2024.zip"
DIR_SHP   = WORKDIR / "seccionado"
SHP_FILE  = DIR_SHP / "SECC_CE_20240101.shp"
CSV_RENTA = WORKDIR / "atlas_renta_30824.csv"

OUT_CSV   = WORKDIR / "renta_centros_educativos.csv"
OUT_XLSX  = WORKDIR / "renta_centros_educativos.xlsx"
...
def geocodifica(centros):
    ...
    filas = []
    for nombre, direccion, curso in centros: # Desempaquetado actualizado
...
# (Las funciones descarga_shapefile, asigna_seccion, descarga_renta y renta_para_cusecs no cambian)

def descarga_shapefile():
    if SHP_FILE.exists(): return
    print(f"→ Descargando shapefile del INE…")
    r = requests.get(URL_SHP, timeout=120)
    ZIP_SHP.write_bytes(r.content)
...
def descarga_renta():
    if CSV_RENTA.exists(): return
    print(f"→ Descargando Atlas de Renta del INE (340 MB)…")
...
def renta_para_cusecs(cusecs_set, año=AÑO):
    indicadores_deseados = ["Renta neta media por hogar", "Renta neta media por persona", "Mediana de la renta por unidad de consumo"]
    trozos = []
    for chunk in pd.read_csv(CSV_RENTA, sep=";", encoding="utf-8", chunksize=200_000, low_memory=False):
        chunk["cusec"] = chunk["Secciones"].astype(str).str.extract(r"^(\d{10})")
        m = (chunk["cusec"].isin(cusecs_set) & (chunk["Periodo"] == año) & (chunk["Indicadores de renta media"].isin(indicadores_deseados)))
        if m.any(): trozos.append(chunk[m])
    
    df = pd.concat(trozos, ignore_index=True)
    pivot = df.pivot_table(index="cusec", columns="Indicadores de renta media", values="Total", aggfunc="first").reset_index()
    for col in pivot.columns:
        if col != "cusec":
            pivot[col] = pivot[col].astype(str).str.replace(".", "", regex=False).str.replace(",", ".", regex=False).pipe(pd.to_numeric, errors="coerce")
    return pivot
